...
BASE_DIR="/usr/local/bin/weather_data_proj"
...
# extract all lines containing temperatures from the weather report and write to file
grep °C "$weather_report" > "$BASE_DIR/temperatures.txt"

# extract the current temperature 
obs_tmp=$(head -1 "$BASE_DIR/temperatures.txt" | tr -s " " | xargs | rev | cut -d " " -f2 | rev)

# extract the forecast for noon tomorrow
fc_temp=$(head -3 "$BASE_DIR/temperatures.txt" | tail -1 | tr -s " " | xargs | cut -d "C" -f2 | rev | cut -d " " -f2 |rev)

# create a tab-delimited record
record=$(echo -e "$year\t$month\t$day\t$hour\t$obs_tmp\t$fc_temp")

# append the record to rx_poc.log
echo $record >> "$BASE_DIR/rx_poc.log"
